...
(function () {
		...
		if (CurrAff) {
			...
			if (SplitLoc == Fields) {
				...
				if (Number(document.activeElement.getAttribute("data-indent")) != 0) {
					for (i = 0; i < Number(document.activeElement.getAttribute("data-indent")) ; i++) {
						IndentOut();
					}
				}
			}
			...
			for (var i = (Number(SplitLoc) + 1) ; i <= Fields; i++) {
	...
	function IndentOut() {
		...
		if (CurrAtt == 0) {
			document.activeElement.style.marginLeft = "1px";
		} else {
		...
		if (DetElNum() < Fields) {
			...
			if (ThisCont < (ContNum + 1)) {
				...
				for (var i = ContNum; i >= ThisCont; i--) {
					...
					var j = 1;
					//I'm going to loop through all the subpoint markers that each subpoint in a contention may have and update their ids
					//j will be my iterator
					while (document.getElementById(i + "Sub" + j) != null) {
						document.getElementById(i + "Sub" + j).id = Number(i + 1) + "Sub" + j;
						j++;
					}
				}
			}
			...
			for (var i = (DetElNum() + 1) ; i <= Fields; i++) {
				...
				SOMETHING HERE IS HORRIBLY BROKEN
				When creating new conts with NEG, if you create a new Cont in AFF after creating one in NEG, then try to create more in NEG, the cont nums and attributes are f***ed up
				...
				*/
				...
				if (!CurrAff && ThisCont == (AffContNum + 1)) {
					ValueHolder += AffContNum;
				}

				if (ValueHolder >= (ThisCont - 1) && Number(document.getElementById("Input" + i).getAttribute("data-cont")) == 0 && (!CurrAff || (CurrAff && (i <= SplitLoc || Number(document.getElementById("Input" + i).getAttribute("data-incont")) != 0)))) {
					if ((ValueHolder == 0 || (!CurrAff && ValueHolder == AffContNum)) && Number(document.getElementById("Input" + i).getAttribute("data-indent")) == 0) {
				...
				if (OtherValueHolder >= (ThisCont - 1) && Number(document.getElementById("Input" + i).getAttribute("data-cont")) != 0 && Number(document.getElementById("Input" + i).getAttribute("data-incont")) == 0) {
		...
		if (DetElNum() < Fields) {
			...
			if (ThisCont < ContNum) {
				...
				for (var i = (ThisCont + 1) ; i <= ContNum; i++) {
					var ContMark = document.getElementById("Cont" + i);
					ContMark.id = "Cont" + (i - 1);
					if ((CurrAff && i <= AffContNum) || !CurrAff) {
		...
		openPicker.pickSingleFileAsync().then(function (file) {
			if (file) {
				...
				Windows.Storage.FileIO.readTextAsync(file).then(function (contents) {
					...
					AffContNum = Number(SplitContent[5]);
					...
					for (var i = 1; i <= (Fields) ; i++) {
						...
						for (var j = 1; j <= Number(document.getElementById("Input" + i).getAttribute("data-rep")) ; j++) {
							...
							if (document.getElementById(CurrRow + "ResponseInput" + j).getAttribute("data-hasreprow") == "true") {
								...
								while (document.getElementById(CurrRow + "ResponseInput" + j + "-" + h) != null) {
									//SavingContent += document.getElementById(CurrRow + "ResponseInput" + j + "-" + h).value + "\n";
									document.getElementById(CurrRow + "ResponseInput" + j + "-" + h).value = SplitContent[j + h];
									h++;
								}
								j = j + h;
							}
